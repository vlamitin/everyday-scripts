...
OKKO_OPTIMUM_FLAGS=-ldflags "-w -s -X main.ElementAlias=${OPTIMUM_COLLECTION} -X main.ElementType=${COLLECTION} -X main.BuildDate=${BUILD_DATE}"
OKKO_NEW_PROMO_FLAGS=-ldflags "-w -s -X main.ElementAlias=${NEW_PROMO_COLLECTION} -X main.ElementType=${COLLECTION} -X main.BuildDate=${BUILD_DATE}"
OKKO_PARAMOUNT_FLAGS=-ldflags "-w -s -X main.ElementAlias=${PARAMOUNT_SUBSCRIPTION} -X main.ElementType=${SUBSCRIPTION} -X main.BuildDate=${BUILD_DATE}"
...
build_okko_films:
	...
	${WINDOWS_ENV} go build ${OKKO_PARAMOUNT_FLAGS} -o bin/${OKKO_FILMS_BINARY}_${PARAMOUNT_SUBSCRIPTION}_${BUILD_DATE}.exe cmd/okko_films_to_csv/films_to_csv.go
	...
	${LINUX_ENV} go build ${OKKO_PARAMOUNT_FLAGS} -o bin/${OKKO_FILMS_BINARY}_${PARAMOUNT_SUBSCRIPTION}_${BUILD_DATE} cmd/okko_films_to_csv/films_to_csv.go
